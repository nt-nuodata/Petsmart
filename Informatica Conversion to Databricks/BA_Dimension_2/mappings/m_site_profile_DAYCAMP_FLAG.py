...
THEN MAX(sp.daycamp_flag)
ELSE MAX(AA.daycamp_flag) --Max aggregation will give us a 1 if at any point during today and the $$p_opendays days before period a store had a daycamp_flag =1
END daycamp_flag 
FROM
--Next subquery will bring up the location_id of the stores that had daycamp related sales on the last $$p_opendays days, and the corresponding daycamp_flag
...
--Below query will bring up all the store locations that didn't have any kind of daycamp-sale during the last $$p_opendays days and set the update of the flag to 0
...
DELETE_DT AS DELETE_DT,
UPDATE_DT AS UPDATE_DT,
LOAD_DT AS LOAD_DT FROM UPD_SITE_PROFILE_2""")
