...
df_2=spark.sql("""
    SELECT
        SP.LOCATION_ID,
        CURRENT_DATE - 1,
        SP.LOYALTY_PGM_CHANGE_DT,
        SP.PETSMART_DMA_CD,
        SP.LOYALTY_PGM_TYPE_ID,
        SP.LOYALTY_PGM_STATUS_ID,
        CURRENT_TIMESTAMP 
    FROM
        SITE_PROFILE SP,
        LOYALTY_PRE LP 
    WHERE
        SP.STORE_NBR = LP.STORE_NBR 
        AND (
            SP.LOYALTY_PGM_CHANGE_DT <> LP.LOYALTY_PGM_CHANGE_DT 
            OR RTRIM(SP.PETSMART_DMA_CD) <> RTRIM(LP.PETSMART_DMA_CD) 
            OR SP.LOYALTY_PGM_TYPE_ID <> LP.LOYALTY_PGM_TYPE_ID 
            OR SP.LOYALTY_PGM_STATUS_ID <> LP.LOYALTY_PGM_STATUS_ID
        )""")

df_2.createOrReplaceTempView("ASQ_Shortcut_To_SITE_PROFILE_2")

# COMMAND ----------
# DBTITLE 1, SITE_MKTG_HISTORY


spark.sql("""INSERT INTO SITE_MKTG_HISTORY SELECT LOCATION_ID AS LOCATION_ID,
LOAD_DT_MINUS_ONE AS SITE_MKTG_EFF_DT,
LOYALTY_PGM_CHANGE_DT AS SITE_MKTG_END_DT,
PETSMART_DMA_CD AS PETSMART_DMA_CD,
LOYALTY_PGM_TYPE_ID AS LOYALTY_PGM_TYPE_ID,
LOYALTY_PGM_STATUS_ID AS LOYALTY_PGM_STATUS_ID,
REAL_LOAD_DT AS LOAD_DT FROM ASQ_Shortcut_To_SITE_PROFILE_2""")