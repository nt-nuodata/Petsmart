...
df_2.createOrReplaceTempView("VENDOR_PROFILE_2")
...
df_3=spark.sql("""
    SELECT
        ...
        FROM
            VENDOR_PROFILE 
        WHERE
            VENDOR_ID = 1802)) PRIMARY_VENDOR_NAME,
        NVL(PSV.PARENT_VENDOR_ID,
        1802) PARENT_VENDOR_ID,
        NVL(PSV.PARENT_VENDOR_NAME,
        (SELECT
            VENDOR_NAME 
        FROM
            VENDOR_PROFILE 
        WHERE
            VENDOR_ID = 1802)) PARENT_VENDOR_NAME,
        PSV.UNIT_NUMERATOR,
        PSV.UNIT_DENOMINATOR,
        PSV.COUNTRY_CD,
        PSV.VENDOR_ARTICLE_NBR,
        NVL(PSV.VENDOR_SUBRANGE_CD,
        ' ') VENDOR_SUBRANGE_CD 
    FROM
        (SELECT
            PVP.SKU_NBR,
        ...
        FROM
            PRIMARY_VENDOR_PRE PVP 
        LEFT JOIN
            VENDOR_PROFILE VP 
                ON PVP.VENDOR_ID = VP.VENDOR_ID 
        LEFT OUTER JOIN
            SKU_VENDOR_DAY SVD 
                ON PVP.SKU_NBR = SVD.SKU_NBR 
                AND PVP.VENDOR_ID = SVD.VENDOR_ID) PSV,
            SKU_PROFILE_RPT R 
    WHERE
...
ZDISCO_STR_WRT_OFF_DT AS ZDISCO_STR_WRT_OFF_DT FROM UPD_SKU_PROFILE_4""")
...
RX_FLAG AS RX_FLAG,
UPDATE_DT AS UPDATE_DT,
LOAD_DT AS LOAD_DT FROM UPD_SKU_PROFILE_4""")
