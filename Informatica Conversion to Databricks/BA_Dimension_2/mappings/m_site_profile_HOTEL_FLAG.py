...
THEN MAX(sp.HOTEL_FLAG)
ELSE MAX(AA.hotel_flag) --Max aggregation will give us a 1 if at any point from today to the $$p_opendays days before, on week period a store had a hotel_flag =1
END hotel_flag 
FROM
--Next subquery will bring up the location_id of the stores that had hotel related sales on the $$p_opendays days, and the corresponding hotel_flag
(SELECT sdss.location_id, week_dt,
       CASE WHEN ((((((  SUM (sdss.sales_amt)
                       - SUM (sdss.return_amt))
                       - SUM (sdss.discount_amt))
                       + SUM (sdss.discount_return_amt))
                       - SUM (sdss.special_sales_amt))
                       + SUM (sdss.special_return_amt))
                       - SUM (sdss.pos_coupon_amt)) > 50
            THEN 1
            ELSE 0
        END hotel_flag
  FROM sales_day_sku_store sdss
  JOIN sku_profile sp
    ON sdss.product_id = sp.product_id
WHERE (sdss.week_dt between (CURRENT_DATE - (date_part ('DOW', CURRENT_DATE) + ($$p_opendays))) AND (CURRENT_DATE - (date_part ('DOW', CURRENT_DATE) - 1)))   
 AND ($$p_hotel)
GROUP BY sdss.location_id, week_dt) AA LEFT JOIN site_profile sp on AA.location_id = sp.LOCATION_ID
GROUP BY AA.location_id 

UNION 

--Below query will bring up all the store locations that didn't have any kind of hotel-sale during the last $$p_opendays days and set the update of the flag to 0
SELECT location_id, 0
  FROM site_profile
WHERE location_id NOT IN (
       SELECT  DISTINCT sdss.location_id
         FROM sales_day_sku_store sdss
         JOIN sku_profile sp
           ON sdss.product_id = sp.product_id
        WHERE (sdss.week_dt between (CURRENT_DATE - (date_part ('DOW', CURRENT_DATE) + ($$p_opendays))) AND (CURRENT_DATE - (date_part ('DOW', CURRENT_DATE) - 1)))
          AND ($$p_hotel)
        GROUP BY sdss.location_id)""")

df_1.createOrReplaceTempView("ASQ_Shortcut_to_SALES_WEEK_CAT_STORE_1")

# COMMAND ----------
# DBTITLE 1, UP_SITE_PROFILE_2
...
df_2.createOrReplaceTempView("UP_SITE_PROFILE_2")
...
DELETE_DT AS DELETE_DT,
UPDATE_DT AS UPDATE_DT,
LOAD_DT AS LOAD_DT FROM UP_SITE_PROFILE_2""")