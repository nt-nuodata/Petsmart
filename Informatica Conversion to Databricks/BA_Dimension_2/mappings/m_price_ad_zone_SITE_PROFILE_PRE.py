...
df_1=spark.sql("""
    SELECT
        DISTINCT NVL(PRICE_AD_ZONE_ID,
        '0'),
        NVL(MAX(PRICE_AD_ZONE_DESC),
        'Not Defined') PRICE_AD_ZONE_DESC 
    FROM
        SITE_PROFILE_PRE 
    GROUP BY
        PRICE_AD_ZONE_ID""")
...
df_2=spark.sql("""
    SELECT
        RPAD(PRICE_AD_ZONE_ID,
        18) AS PRICE_AD_ZONE_ID,
        PRICE_AD_ZONE_DESC AS PRICE_AD_ZONE_DESC,
        Monotonically_Increasing_Id AS Monotonically_Increasing_Id 
    FROM
        Shortcut_To_SITE_PROFILE_PRE_1""")

df_2.createOrReplaceTempView("EXPTRANS_2")

# COMMAND ----------
# DBTITLE 1, PRICE_AD_ZONE


spark.sql("""INSERT INTO PRICE_AD_ZONE SELECT PRICE_AD_ZONE_ID AS PRICE_AD_ZONE_ID,
PRICE_AD_ZONE_DESC AS PRICE_AD_ZONE_DESC FROM EXPTRANS_2""")