...
# COMMAND ----------
# DBTITLE 1, ASQ_Shortcut_To_SALES_DAY_SKU_STORE_VEND_3
...
df_3=spark.sql("""
    SELECT
        SDSS.LOCATION_ID,
        CASE 
            WHEN (SUM(SDSS.SALES_AMT) - SUM(SDSS.RETURN_AMT) - SUM(SDSS.DISCOUNT_AMT) + SUM(SDSS.DISCOUNT_RETURN_AMT) - SUM(SDSS.SPECIAL_SALES_AMT) + SUM(SDSS.SPECIAL_RETURN_AMT) - SUM(SDSS.POS_COUPON_AMT)) > 50 THEN MIN(SDSS.DAY_DT) 
            ELSE NULL 
        END HOTEL_OPEN_DT 
    FROM
        SALES_DAY_SKU_STORE SDSS,
        SKU_PROFILE SP,
        SITE_PROFILE SITE 
    WHERE
        $$p_hotel_open 
        AND SDSS.DAY_DT > CURRENT_DATE - 30 
        AND SDSS.PRODUCT_ID = SP.PRODUCT_ID 
        AND SITE.LOCATION_ID = SDSS.LOCATION_ID 
        AND (
            SITE.HOTEL_OPEN_DT = TO_DATE(12319999, 'MMDDYYYY') 
            OR SITE.HOTEL_OPEN_DT IS NULL
        ) 
    GROUP BY
        SDSS.LOCATION_ID 
    HAVING
        (
            SUM(SDSS.SALES_AMT) - SUM(SDSS.RETURN_AMT) - SUM(SDSS.DISCOUNT_AMT) + SUM(SDSS.DISCOUNT_RETURN_AMT) - SUM(SDSS.SPECIAL_SALES_AMT) + SUM(SDSS.SPECIAL_RETURN_AMT) - SUM(SDSS.POS_COUPON_AMT)
        ) > 50""")

df_3.createOrReplaceTempView("ASQ_Shortcut_To_SALES_DAY_SKU_STORE_VEND_3")
...
DELETE_DT AS DELETE_DT,
UPDATE_DT AS UPDATE_DT,
LOAD_DT AS LOAD_DT FROM ASQ_Shortcut_To_SALES_DAY_SKU_STORE_VEND_3""")
